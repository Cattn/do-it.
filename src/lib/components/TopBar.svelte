<script lang="ts">
    // bro. lemme rant for a sec.
    // why the FUCK IS SVELTE GIVING ME RANDOM ERRORS
...
<style>
    .big-button :global(button) {
        border-radius: 4rem;
        width: 12rem;
        height: 4rem;
    }
    .big-button :global(button svg) {
        width: 2.4rem;
        height: 2.4rem;
    }
    .big-button-2 :global(button) {
        border-radius: 4rem;
        width: 4rem;
        height: 4rem;
        gap: 0;
    }
    .big-button-2 :global(button svg) {
        width: 2rem;
        height: 2rem;
    }
    .button-text {
        font-family: "Roboto Flex", sans-serif;
        font-optical-sizing: 41;
        font-weight: 820;
        font-style: normal;
        text-align: right;
